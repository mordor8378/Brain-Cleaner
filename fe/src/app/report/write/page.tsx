...
export default function ReportPage({
  ...
  const handleSubmit = async () => {
     if (!reason || reason.length < 5) { // 백엔드 validation (5자 이상) 반영
         ...
         if (reason.length > 1000) { // 백엔드 validation (1000자 이하) 반영
           alert('신고 사유는 1000자 이하로 입력해주세요.');
           if (reasonTextareaRef.current) {
             reasonTextareaRef.current.focus();
           }
           return;
        ...
        try {
          ...
          const apiUrl = 'http://localhost:8090/api/v1/reports';
          ...
          // 응답 처리
          if (response.ok || response.status === 201) { // 성공 (200 OK 또는 201 Created)
