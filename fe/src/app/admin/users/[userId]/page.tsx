...
const getRoleDisplayName = (roleCode: string | undefined): string => {
  switch (roleCode) {
    ...
    case 'ROLE_USER_SPROUT': return '디톡스새싹';
    case 'ROLE_USER_TRAINEE': return '절제수련생';
    case 'ROLE_USER_EXPLORER': return '집중탐험가';
    case 'ROLE_USER_CONSCIOUS': return '선명한의식';
    case 'ROLE_USER_DESTROYER': return '도파민파괴자';
    case 'ROLE_USER_CLEANER': return '브레인클리너';
    default: return roleCode || '정보 없음'; // 코드가 없거나 모르는 경우
  }
};
...
const getStatusDisplayName = (statusCode: string | undefined): string => {
  switch (statusCode) {
    ...
    default: return statusCode || '정보 없음';
  }
};
...
export default function UserDetailPage() {
  ...
  useEffect(() => {
    ...
    console.log('[UserDetailPage useEffect] Effect 실행! 현재 params 객체:', params);

    const fetchUserData = async () => {
        // userId가 유효한지 먼저 확인
        if (!userId || userId === 'undefined' || isNaN(Number(userId))) {
                  console.error('[UserDetailPage fetchUserData] 유효하지 않은 userId:', userId);
      ...
      try {
          const backendApiUrl = `http://localhost:8090/api/admin/users/${params.userId}`;
        ...
        if (!response.ok) {
             if (response.status === 401) throw new Error('인증되지 않은 사용자입니다. 로그인이 필요합니다.');
             if (response.status === 403) throw new Error('접근 권한이 없습니다 (관리자 아님).');
             if (response.status === 404) throw new Error('해당 사용자를 찾을 수 없습니다.');
  ...
  const handleApplyChanges = async () => {
    ...
    const roleChanged = selectedRole !== user.role;
     ...
     console.log(`[handleApplyChanges] userId: ${userId}, status: ${selectedStatus}, role: ${selectedRole} 변경 요청`);
     setIsUpdating(true);
     setError(null);
     ...
     if (statusChanged) {
         console.log(`[handleApplyChanges] 상태 변경 API 호출 준비: ${selectedStatus}`);
         const statusUrl = `http://localhost:8090/api/admin/users/${userId}/status`;
         apiCalls.push(fetch(statusUrl, {
     ...
     if(roleChanged) {
         console.log(`[handleApplyChanges] 역할 변경 API 호출 준비: ${selectedRole}`);
         const roleUrl = `http://localhost:8090/api/admin/users/${userId}/role`;
         apiCalls.push(fetch(roleUrl, {
           method: 'PUT', // ★★★ PUT 방식 사용 ★★★
           headers: { 'Content-Type': 'application/json' },
           credentials: 'include',
          ...
          // 3. 준비된 API 호출들을 동시에 실행
               ...
               if (allSuccess) {
                 ...
                 // 임시로 상태 직접 업데이트 (fetchUserData 호출이 더 확실함)
                  setUser(prevUser => prevUser ? { ...prevUser, status: selectedStatus, role: selectedRole } : null);
                 ...
                 const errorMessage = errorData?.message || `오류 발생 (상태 코드: ${failedResponse?.status})`;
                 console.error('[handleApplyChanges] 일부 또는 전체 업데이트 실패:', failedResponse?.status, errorData);
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex">
              ...
              <span className="font-medium">
              {user.updatedAt ? new Date(user.updatedAt).toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }).replace(/\.$/, '') : '-'}
              </span>
            </div>
            <div className="flex">
            ...
            <div className="flex">
              ...
              <span className="badge bg-primary/10 text-primary">{getRoleDisplayName(user.role)}</span>
            </div>
            <div className="flex">
              <span className="w-32 text-gray-500">상태</span>
              <span className="badge bg-green-100 text-green-800">{getStatusDisplayName(user.status)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded shadow-sm p-6">
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr>
                  ...
                  <th className="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">변경 사유</th>
                  <th className="px-6 py-3 bg-gray-50 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">변경량</th>
                </tr>
              </thead>
          ...
          <div className="flex items-center justify-between mt-4">
            <div className="text-sm text-gray-500">
              총 {pointHistoryTotalItems}개 항목 중 {pointHistoryCurrentPage * 10 - 9}-{Math.min(pointHistoryCurrentPage * 10, pointHistoryTotalItems)}
            </div>
        ...
        <div className="bg-white rounded shadow-sm p-6">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label htmlFor="statusSelect" className="block text-lg font-semibold text-gray-600 mb-3">상태 변경</label>
                <div className="relative">
                  ...
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                        <span className="w-5 h-5 flex items-center justify-center">
                          <i className="ri-arrow-down-s-line"></i> {/* 아이콘 클래스 */}
                        </span>
                      </div>
                </div>
              </div>
              <div>
                <label htmlFor="roleSelect" className="block text-lg font-semibold text-gray-600 mb-3">역할 변경</label>
                <div className="relative">
                  <select
                    id="roleSelect"
                    value={selectedRole}
            ...
            <div className="flex justify-end space-x-3 mt-6">
              <button
                ...
                disabled={isUpdating || (selectedStatus === user?.status && selectedRole === user?.role)}
              ...
              >
                <div className="flex items-center">
                  {isUpdating && <span className="w-4 h-4 border-2 border-gray-500 border-t-transparent rounded-full animate-spin mr-2"></span>}
                  <span>적용</span>
                </div>
              </button>
              <button
