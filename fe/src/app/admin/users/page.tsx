...
export default function AdminUsersPage() {
  ...
  const fetchUsers = useCallback(async (page = 1) => {
    ...
    console.log(`[fetchUsers] ${page} 페이지 (API 요청 시 ${apiPage}) 데이터 로딩 시작...`);
    setIsLoading(true);
    ...
    try {
      ...
      const backendApiUrl = 'http://localhost:8090/api/admin/users';
      console.log(`[fetchUsers] Request URL: ${backendApiUrl}?${queryParams}`);
      ...
      if (!response.ok) {
        ...
        console.error(`[fetchUsers] API Error Response (${response.status}):`, errorText);
  ...
  const getRoleClass = (role: string) => {
    switch (role) {
      case '디톡스새싹': return 'bg-green-100 text-green-800';
      case '절제수련생': return 'bg-blue-100 text-blue-800';
      case '집중탐험가': return 'bg-indigo-100 text-indigo-800';
      case '선명한의식': return 'bg-purple-100 text-purple-800';
      case '도파민파괴자': return 'bg-pink-100 text-pink-800';
      case '브레인클리너': return 'bg-yellow-100 text-yellow-800';
      case '관리자': return 'bg-red-100 text-red-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };
  const getStatusClass = (status: string) => {
    switch (status) {
      case '계정활성': return 'bg-green-100 text-green-800';
      case '계정정지': return 'bg-yellow-100 text-yellow-800';
      case '계정삭제': return 'bg-red-100 text-red-800';
      default: return 'bg-gray-100 text-gray-800';
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        >
          <span className={`w-5 h-5 flex items-center justify-center ${isLoading ? 'animate-spin' : ''}`}>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              key="nickname-input"
              type="text" id="nickname" value={searchParams.nickname}
          ...
          <div>
            ...
            <input
              key="email-input"
              type="email" id="email" value={searchParams.email}
          ...
          <div>
            <label htmlFor="role" className="block text-sm font-medium text-gray-700 mb-1">역할</label>
            <div className="relative">
              <select id="role" value={searchParams.role}
              ...
              <div className="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">
                <span className="w-5 h-5 flex items-center justify-center text-gray-500"><i className="ri-arrow-down-s-line"></i></span>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              <select id="status" value={searchParams.status}
        ...
        <div className="flex justify-end gap-2">
          <button onClick={handleReset} disabled={isLoading} className="px-4 py-2 border border-gray-300 text-gray-700 !rounded-button whitespace-nowrap disabled:opacity-50">초기화</button>
          <button onClick={handleSearch} disabled={isLoading} className="px-4 py-2 bg-primary text-white !rounded-button whitespace-nowrap disabled:opacity-50">검색</button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between flex-wrap gap-2">
          ...
          <div className="text-sm text-gray-500">총 {pagination.totalItems} 명</div>
        </div>
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <div className="text-center p-16 text-gray-500">표시할 회원이 없습니다.</div>
          ) : (
            <table className="w-full min-w-[768px]">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">닉네임</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">이메일</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">역할</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">상태</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider whitespace-nowrap">가입일</th>
                </tr>
              </thead>
                ...
                {users.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getRoleClass(user.role)}`}>
                        {user.role}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getStatusClass(user.status)}`}>
                        {user.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {user.createdAt ? new Date(user.createdAt).toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' }) : '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {!isLoading && pagination.totalPages > 1 && (
           <div className="px-6 py-4 flex items-center justify-center border-t border-gray-200">
              <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                ...
                >
                  <span className="w-5 h-5 flex items-center justify-center"><i className="ri-arrow-left-s-line" aria-hidden="true"></i></span>
                </button>
                {Array.from({ length: pagination.totalPages }, (_, i) => i + 1)
                    .filter(page => Math.abs(page - pagination.currentPage) < 3 || page === 1 || page === pagination.totalPages)
                    .map((page, index, arr) => (
                        <React.Fragment key={page}>
                         {(index > 0 && page - arr[index-1] > 1 && page !== pagination.totalPages && arr[index-1] !== 1) && (
                         ...
                         {(index < arr.length - 1 && arr[index+1] - page > 1 && page !== 1 && arr[index+1] !== pagination.totalPages) && (
                           <span className="relative inline-flex items-center px-4 py-2 border border-gray-300 bg-white text-sm font-medium text-gray-700">...</span>
                         )}
                        </React.Fragment>
                    ))
                ...
                >
                  <span className="w-5 h-5 flex items-center justify-center"><i className="ri-arrow-right-s-line" aria-hidden="true"></i></span>
                </button>
              </nav>
          </div>
        )}
