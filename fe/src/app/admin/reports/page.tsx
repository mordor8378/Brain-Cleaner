...
const AdminReportPage: React.FC = () => {
  ...
  const fetchReports = async ({ pageParam = 0 }): Promise<ReportPage> => {
    ...
    const apiUrl = `http://localhost:8080/api/admin/reports?page=${pageParam}&size=10&sort=createdAt,asc`;
  ...
  const approveMutation = useMutation({
    mutationFn: async (reportId: number) => {
      const apiUrl = `http://localhost:8080/api/admin/reports/${reportId}/status`;
  ...
  const deleteMutation = useMutation({
    ...
    mutationFn: async (postId: number) => {
      ...
      const postDeleteApiUrl = `http://localhost:8080/api/admin/posts/${postId}`;
