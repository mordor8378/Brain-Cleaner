...
const AdminAuthGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const validateAdminRole = async () => {
      try {
        ...
        if (userData.role !== 'ROLE_ADMIN') {
          console.log('[AdminAuthGuard] 서버 확인 결과 관리자 아님. 리다이렉트 실행');
    ...
    if (!loading) {
      console.log('[AdminAuthGuard:useEffect] UserContext 로딩 완료. validateAdminRole 호출.');
      validateAdminRole();
    } else {
      console.log('[AdminAuthGuard:useEffect] UserContext 로딩 중... validateAdminRole 대기.');
    }
  ...
  if (loading || isValidating) {
    console.log(`[AdminAuthGuard] 스피너 표시 (loading: ${loading}, isValidating: ${isValidating})`);
    return (
